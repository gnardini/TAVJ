...
public class BitBuffer {
	...
	public BitBuffer() : this(new MemoryStream(CAPACITY)){
	}
	...
	public byte[] GetByteArray(){
		int size = GetAvailableByteCount();
	...
	public byte GetByte() {
		// use sbyte to match java signed byte behavior
		return (byte)GetBits(0, byte.MaxValue);
	}

	public byte GetSignedByte() {
		// use sbyte to match java signed byte behavior
		return (byte)GetBits(sbyte.MinValue, sbyte.MaxValue);
	}
